...
export default function Members() {
  ...
  const handleCheckIn = async (memberId, memberName) => {
    try {
      setActionLoading(true); setActionError(null);
      await api.post('/api/attendance', { member_id: memberId, date: new Date().toISOString() });
      setActionSuccess(`${memberName} checked in successfully!`);
      setTimeout(() => setActionSuccess(null), 3000);
    } catch (err) {
      ...
      <div className="grid grid-cols-3 gap-sm lg:gap-gutter">
        ...
        >
          ...
          <div className="absolute right-4 top-0 bottom-0 flex items-center justify-center font-bold select-none group-hover:scale-110 transition-transform duration-700 pointer-events-none">
            ...
            <span className="material-symbols-outlined text-[64px] leading-none block relative z-10 text-error/15 group-hover:text-error/50 transition-all duration-700 group-hover:drop-shadow-[0_0_25px_rgba(239,68,68,0.8)]">cancel</span>
          </div>
          <div className="flex items-center justify-between relative z-10">
            <div>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-sm px-md text-right whitespace-nowrap">
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); handleCheckIn(member.id, member.name); }}
                        disabled={actionLoading}
                        className="w-8 h-8 inline-flex items-center justify-center text-on-surface/20 hover:text-green-500 hover:bg-green-500/10 transition-all disabled:opacity-30 mr-1"
